...
- **No language-specific logic** outside of parser modules. Parsers (`src/agent5/parser.rs`, `src/ecosystems/`) are the ONLY place for language-specific handling.
...
### Environment
- OpenAI API key lives at `~/.openai` (source it or read from there)
- Test config at `/tmp/split-config.toml`
...
This is a Rust CLI project. Always use `cargo test`, `cargo clippy`, and `cargo build` for validation. When fixing tests, run the full test suite afterward to check for test pollution or regressions.
...
When installing or depending on external packages (npm, cargo, etc.), always pin to specific known-working versions. Never use `@latest` for dependencies in hooks or MCP servers without verifying compatibility first.
