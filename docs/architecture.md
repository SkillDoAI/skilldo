...
All `<!-- SKILLDO-* -->` comments are stripped from the final output and logged for debugging. CONFLICT notes log at `info` level, UNVERIFIED notes at `warn`. Use `RUST_LOG=debug` or `RUST_LOG=warn` to see them.
