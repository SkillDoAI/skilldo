...
* Note: Copy-on-Write behavior changed in v3.0 - selections behave as independent objects for mutation
...
# Convert to datetime
df = pd.DataFrame({'date_string': ['2024-01-01', '2024-01-02']})
df['date'] = pd.to_datetime(df['date_string'])

# Convert to timedelta
df = pd.DataFrame({'duration_string': ['1 days', '2 days']})
...
```python
df = pd.DataFrame({'A': [1, 2, 3]})
df2 = df[['A']]  # CoW semantics prevent mutation propagation to df
df2.iloc[0, 0] = 99  # Does not modify original df
```
**Migration:** Copy-on-Write (CoW) is now the default and only mode in pandas 3.0. Subset mutations do not propagate back to the original object. If you need to modify the original DataFrame, use direct assignment with `.loc[]` or `.iloc[]` rather than chaining operations.
...
### Index Types
- **Index(data=None, dtype=None, copy=None, name=None, tupleize_cols=True)** - Immutable sequence used for indexing and alignment
...
### Scalars
- **Timestamp(ts_input=None, year=None, month=None, day=None, hour=None, minute=None, second=None, microsecond=None, tzinfo=None, *, nanosecond=None, tz=None, unit=None, fold=None)** - Pandas replacement for datetime.datetime with nanosecond precision
...
### Data Manipulation
- **concat(objs, axis=0, join='outer', ignore_index=False, keys=None, levels=None, names=None, verify_integrity=False, sort=False, copy=<no_default>)** - Concatenate pandas objects along a particular axis
- **merge(left, right, how='inner', on=None, left_on=None, right_on=None, left_index=False, right_index=False, sort=False, suffixes=('_x', '_y'), copy=<no_default>, indicator=False, validate=None)** - Merge DataFrame or named Series objects with database-style join
- **pivot_table(data, values=None, index=None, columns=None, aggfunc='mean', fill_value=None, margins=False, dropna=True, margins_name='All', observed=True, sort=True)** - Create spreadsheet-style pivot table as DataFrame
...
### Type Conversion
- **to_datetime(arg, errors='raise', dayfirst=False, yearfirst=False, utc=False, format=None, exact=<no_default>, unit=None, origin='unix', cache=True)** - Convert argument to datetime
