...
### ⚠️ Deprecated: Using `np.chararray`
```python
import numpy as np

def main() -> None:
    # Deprecated API; avoid in new code.
    arr = np.chararray((2,), itemsize=5)
    arr[:] = [b"abc", b"xyz"]
    print(arr)
...
### Right: Prefer `numpy.strings` operations or regular ndarrays with string dtype
```python
import numpy as np

def main() -> None:
    arr = np.array(["AbC", "XyZ"], dtype=np.str_)
    lowered = np.strings.lower(arr)
    print(lowered)
...
- ⚠️ `np.chararray` is deprecated; migrate to `numpy.strings` operations or standard `ndarray` string dtypes.
- ⚠️ `numpy.distutils` is deprecated/compatibility-only and limited on newer Python versions; migrate build workflows to setuptools/meson-python.
...
- Replace `np.chararray` usage in runtime code with non-deprecated string array operations.
- If you still depend on `numpy.distutils` for building extensions, plan migration to modern Python build backends.
- See [NumPy changelog](https://numpy.org/doc/stable/release) for details on API adjustments in 2.x.

## API Reference

- **numpy.array**  
  `array(object, dtype=None, *, copy=True, order='K', subok=False, ndmin=0, ndmax=0, like=None) -> ndarray`
- **numpy.asarray**  
  `asarray(a, dtype=None, order=None, *, device=None, copy=None, like=None) -> ndarray`
- **numpy.arange**  
  `arange(start_or_stop, /, stop=None, step=1, *, dtype=None, device=None, like=None) -> ndarray`
- **numpy.linspace**  
  `linspace(start, stop, num=50, endpoint=True, retstep=False, dtype=None, axis=0, *, device=None) -> ndarray | tuple[ndarray, float]`
- **numpy.zeros**  
  `zeros(shape, dtype=None, order='C', *, device=None, like=None) -> ndarray`
- **numpy.ones**  
  `ones(shape, dtype=None, order='C', *, device=None, like=None) -> ndarray`
- **numpy.empty**  
  `empty(shape, dtype=None, order='C', *, device=None, like=None) -> ndarray`
- **numpy.dtype**  
  `dtype(dtype, align=False, copy=False, **kwargs) -> dtype`
- **numpy.reshape**  
  `reshape(a, /, shape, order='C', *, copy=None) -> ndarray`
- **numpy.concatenate**  
  `concatenate(arrays, /, axis=0, out=None, *, dtype=None, casting='same_kind') -> ndarray`
- **numpy.stack**  
  `stack(arrays, axis=0, out=None, *, dtype=None, casting='same_kind') -> ndarray`
- **numpy.where**  
  `where(condition, x=None, y=None, /) -> ndarray | tuple[ndarray, ...]`
- **numpy.sum**  
  `sum(a, axis=None, dtype=None, out=None, keepdims=<no value>, initial=<no value>, where=<no value>) -> scalar or ndarray`
- **numpy.mean**  
  `mean(a, axis=None, dtype=None, out=None, keepdims=<no value>, *, where=<no value>) -> scalar or ndarray`
- **numpy.std**  
  `std(a, axis=None, dtype=None, out=None, ddof=0, keepdims=<no value>, *, where=<no value>, mean=<no value>, correction=<no value>) -> scalar or ndarray`
- **numpy.min**  
...
- **numpy.chararray** ⚠️ Deprecated  
  `chararray(...) -> numpy.chararray`
- **numpy.distutils** ⚠️ Deprecated compatibility namespace  
  `distutils (lazy submodule attribute; deprecated for modern builds)`
...
- `np.chararray` is deprecated and should be avoided in new code.
- `numpy.distutils` remains as a deprecated compatibility/build subsystem, not a recommended runtime API.
- No major user-facing removals in the core array API used above; most changes are improved error reporting, typing, and deprecation guidance.
