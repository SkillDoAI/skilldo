...
* Mode can be `'before'`, `'after'` (default), `'wrap'`
...
- Dynamic imports of deprecated symbols like `FieldValidationInfo` and `GenerateSchema` via `pydantic.__getattr__` emit deprecation warnings. ⚠️
...
- If you use `Color` from `pydantic.color`, migrate to `pydantic_extra_types.color.Color` when convenient (`Color` in `pydantic` is deprecated since v2.0). ⚠️
...
- **field_validator(*fields, mode='after')** - Decorator for field-level validation; modes: 'before', 'after', 'wrap'
...
- **AliasPath(first_arg, *args)** - Define nested alias paths for validation
- **AliasChoices(first_choice, *choices)** - Define multiple alias alternatives for validation
- **AliasGenerator(alias=None, validation_alias=None, serialization_alias=None)** - Configure advanced alias generation for validation/serialization