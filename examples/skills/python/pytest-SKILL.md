...
* ⚠️ Prefer `tmp_path`/`tmp_path_factory` (`pathlib.Path`) over legacy `tmpdir`/`tmpdir_factory` (py.path-style).
* ⚠️ `testdir`/`Testdir` APIs are legacy; prefer `pytester` for new code.
...
- **pytest.main** - Run pytest programmatically; signature: `main(args: list[str] | os.PathLike[str] | None = None, plugins: Sequence[str | object] | None = None) -> int | ExitCode`.
- **pytest.ExitCode** - Exit status enum: `OK`, `TESTS_FAILED`, `INTERRUPTED`, `INTERNAL_ERROR`, `USAGE_ERROR`, `NO_TESTS_COLLECTED`.
- **pytest.console_main** - Console entrypoint; signature: `console_main() -> int`.
- **pytest.register_assert_rewrite** - Register modules/packages for assertion rewriting; signature: `register_assert_rewrite(*names: str) -> None`.