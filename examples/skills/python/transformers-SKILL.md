---
name: transformers
description: State‑of‑the‑art library for natural language processing, vision, and speech, offering pretrained models, tokenizers, pipelines, and utilities.
version: 5.3.0
ecosystem: python
license: Apache-2.0
...
def main() -> None:
    # Pipelines require a DL backend (PyTorch / TensorFlow / Flax).
    # In tokenizer‑only installs (common in minimal CI), calling `pipeline()`
    # will fail because model classes are unavailable.
    #
    # IMPORTANT: backend availability helpers are top‑level in transformers.
    # As of transformers 5.3.0 only `is_torch_available` is provided.
    # TensorFlow/JAX backends can be detected via optional imports if needed.
    try:
        has_backend = transformers.is_torch_available()
    except Exception:
        has_backend = False

    if not has_backend:
        print("No DL backend available (PyTorch). Skipping pipeline creation.")
        return

    # Some transformers versions reference `torch` internally when `dtype` is a str,
    # even if you don't pass `dtype`. Importing torch when available avoids
    # NameError in those environments.
    try:
        import torch  # noqa: F401
    except Exception:
        # If the active backend is TF/Flax, torch may not be installed; proceed.
        pass

    # Always provide an explicit model id to avoid large default downloads.
    text_gen = pipeline(task="text-generation", model="sshleifer/tiny-gpt2")
    out = text_gen("The secret to baking a really good cake is ", max_new_tokens=40)
    print(out[0]["generated_text"])

    # Use a tiny image model to reduce download size.
    img_cls = pipeline(task="image-classification", model="hf-internal-testing/tiny-random-vit")
...
* Pipelines require a backend; if you only installed `transformers` (no `torch`/`tensorflow`/`jax`), guard or install `transformers[torch]`, `transformers[tf]`, or `transformers[jax]`.
...
def main() -> None:
    # Pipelines require a DL backend (PyTorch / TensorFlow / Flax).
    # In tokenizer‑only installs, calling `pipeline()` will fail because
    # model classes are unavailable.
    #
    # IMPORTANT: backend availability helpers are top‑level in transformers.
    # As of transformers 5.3.0 only `is_torch_available` is provided.
    try:
    ...
    if not has_backend:
        print("No DL backend available (PyTorch). Skipping chat pipeline example.")
        return

    # See note in the pipeline example: some versions reference `torch` internally.
    try:
        import torch  # noqa: F401
    except Exception:
        pass

    # Use a public, non‑gated instruct model so this runs without authentication.
    chat = [
        {"role": "system", "content": "You are a concise assistant."},
        {"role": "user", "content": "Give me 3 ideas for a weekend trip from Paris."},
    ]

    # IMPORTANT: keep kwargs minimal for cross‑backend robustness.
    # Some versions/backends may error if `device_map`/`dtype` are provided.
    pipe = pipeline(
...
def main() -> None:
    ...
    try:
        ...
        # ⚠️ `additional_special_tokens` is deprecated in v5; use `extra_special_tokens` instead.
...
* ⚠️ `additional_special_tokens` is deprecated in v5 and converted to `extra_special_tokens` (prefer `extra_special_tokens` for new code).
...
- Special tokens:
  ...
  - ⚠️ Note: `additional_special_tokens` is deprecated in v5 and converted to `extra_special_tokens` (prefer `extra_special_tokens` for new code).
...
vqa = pipeline(task="visual-question-answering", model="Salesforce/blip-vqa-base")
image_url = "https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/transformers/tasks/idefics-few-shot.jpg"
print(
    vqa(
        image=image_url,
...
```python
from transformers import PretrainedConfig  # ⚠️ Deprecated naming (legacy alias)
...
3. **Special tokens parameter naming**: ⚠️ `additional_special_tokens` is deprecated in favor of `extra_special_tokens`.
   - **Action**: Replace `{"additional_special_tokens": [...]}` with `{"extra_special_tokens": [...]}` when calling `tokenizer.add_special_tokens(...)`.
   - **Note**: This is a change in the *dictionary key* passed via `special_tokens_dict`, not a change to the `add_special_tokens(...)` function parameters.

4. **Config class alias**: ⚠️ `PretrainedConfig` remains as a deprecated legacy alias for `PreTrainedConfig`.
   - **Action**: Prefer importing/using `PreTrainedConfig`.
...
- **transformers.pipeline(task, model, \*\*kwargs)** – Create a task‑specific `Pipeline`; key kwargs include `device_map=`, `dtype=`, `trust_remote_code=`, `revision=`, `device=`, `model_kwargs=`.
...
- **transformers.AutoTokenizer.from_pretrained(pretrained_model_name_or_path, \*\*kwargs)** – Load tokenizer from a model id or local directory; accepts tokenizer‑specific kwargs.
...
- **PreTrainedTokenizerBase.add_special_tokens(special_tokens_dict, replace_extra_special_tokens=True)** – Add special tokens; supports `extra_special_tokens` (preferred) and deprecated `additional_special_tokens`.
...
- **transformers.CompileConfig** – Configuration for compilation‑related generation settings.
...
- **transformers.AsyncTextIteratorStreamer** – Async iterator‑based streamer for token‑by‑token generation.
...
- **transformers.quantizers.register_quantizer(name)** – Decorator to register custom quantizers.
- **transformers.quantizers.register_quantization_config(method)** – Decorator to register custom quantization configs.
...
- **transformers.enable_full_determinism(seed, warn_only=False)** – Enable more deterministic behavior across libraries (where supported).
...
- **transformers.utils.metrics.attach_tracer(tracer_name_template=None)** – Class decorator that attaches a tracer to instances (`self.tracer`); optionally customize the tracer name template.
- **transformers.utils.metrics.traced(*, span_name=None, standalone=False, additional_attributes=None)** – Decorator for tracing functions/methods; supports custom span names and attributes.