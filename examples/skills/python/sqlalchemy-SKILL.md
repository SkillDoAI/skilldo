...
def main() -> None:
    # SQLAlchemy 2.x is already "future"; no `future=True` needed.
    engine = create_engine("sqlite+pysqlite:///:memory:")

    # Use `engine.begin()` for an explicit transaction boundary.
    ...
    # Read in a separate connection (no transaction required for read‑only).
    with engine.connect() as conn:
        rows = conn.execute(
            text("SELECT id, name FROM user_account ORDER BY id")
        ).all()
        print(rows)
...
def main() -> None:
    ...
    with engine.begin() as conn:
        ...
        conn.execute(stmt, {"old_name": "bob", "new_name": "robert"})

        # SELECT
        names = conn.execute(
            select(user_account.c.id, user_account.c.name).order_by(user_account.c.id)
        ).all()
        print(names)

        # DELETE
        conn.execute(
            delete(user_account).where(user_account.c.name == bindparam("to_delete")),
            {"to_delete": "alice"},
        )

    with engine.connect() as conn:
        remaining = conn.execute(
            select(user_account.c.name).order_by(user_account.c.name)
        ).scalars().all()
        print(remaining)
...
def main() -> None:
    ...
    # Eager load to avoid relying on lazy‑load behavior in an example.
    with Session(engine) as session:
...
def main() -> None:
    # `future=True` is deprecated in SQLAlchemy 2.0; omit it.
    engine = create_engine("sqlite+pysqlite:///:memory:")
    Base.metadata.create_all(engine)
...
def main() -> None:
    ...
    with Session(engine) as session:
        ...
        print(user.keywords)  # Access keywords directly without going through the association table
...
- **Core**: `select()`, `insert()`, `update()`, `delete()` are the preferred entry points.  
- **ORM**: `DeclarativeBase`, `Mapped[…]`, `mapped_column()` replace the older `declarative_base()` and plain `Column` usage.  
- **Transactions**: Implicit autocommit is removed; use `engine.begin()` (Core) or `Session.commit()` (ORM).  
- **Future flag**: The `future=True` argument to `create_engine()` and `Session()` is deprecated and ignored in 2.0.  
- **Legacy Query API**: `session.query()` and `Query` objects are soft‑deprecated; use `Session.execute(select(...))` and `.scalars()`.  
...
* Modern alternative: `Session.execute(select(Model).where(...)).scalars()`  
* Migration guidance: replace `session.query(Model).filter(...)` with the 2.0 pattern.
...
### ShardedSession legacy API ⚠️ Deprecation
The `id_chooser` and `query_chooser` parameters of `ShardedSession` are deprecated in favor of `identity_chooser` and `execute_chooser`. Use the modern API for new code.

```python
# Legacy Sharding API – id_chooser & query_chooser (deprecated) ⚠️
from sqlalchemy.ext.horizontal_shard import ShardedSession
from sqlalchemy.orm import sessionmaker

# configure with deprecated parameters (will emit deprecation warnings)
sharded_session = sessionmaker(class_=ShardedSession, autoflush=True)
sharded_session.configure(
    shards={"north_america": db1, "asia": db2, "europe": db3, "south_america": db4},
    shard_chooser=shard_chooser,
    id_chooser=id_chooser,          # ← deprecated
    query_chooser=query_chooser,    # ← deprecated
)
```

## API Reference

- **sqlalchemy.create_engine(url, \*\*kwargs)** – Create an `Engine`.  
  **Signature (2.0)**: `create_engine(url: str | URL, echo: bool = False, \*\*kwargs) -> Engine`  
  *The `future` parameter is removed; passing it raises a deprecation warning.*
...
- **sqlalchemy.text(sql_text, bindparams=None, \*\*kw)** – Create a textual SQL statement supporting bound parameters (`:name`).

- **sqlalchemy.select(\*entities, \*\*kwargs) → Select** – Build a SELECT statement from tables/columns/ORM entities.

- **sqlalchemy.insert(table) → Insert** – Build an INSERT statement.

- **sqlalchemy.update(table) → Update** – Build an UPDATE statement.

- **sqlalchemy.delete(table) → Delete** – Build a DELETE statement.

- **sqlalchemy.bindparam(key, \*\*options)** – Define an explicit bound parameter for SQL constructs.

- **sqlalchemy.Column(\*args, primary_key=False, nullable=True, \*\*kwargs)** – Define a table column (Core API). For ORM mappings, prefer `mapped_column()`.

- **sqlalchemy.ForeignKey(column, \*\*options)** – Define a foreign key constraint.

- **sqlalchemy.Integer()** – Integer column type.

- **sqlalchemy.String(length=None, collation=None, \*\*kw)** – String/VARCHAR column type.

- **sqlalchemy.MetaData(\*\*options)** – Container for schema constructs like `Table`.

- **sqlalchemy.Table(name, metadata, \*columns, \*\*kwargs)** – Represent a database table in Core.

- **sqlalchemy.event.listen(target, identifier, fn, \*args, \*\*kw)** – Register an event listener on a target object.

- **sqlalchemy.event.listens_for(target, identifier, \*\*kw)** – Decorator to register an event listener.

- **sqlalchemy.orm.Session(bind=None, \*\*options)** – ORM session (unit of work / identity map); use `commit()` to persist.
...
- **sqlalchemy.orm.Session.execute(statement, params=None, \*\*kw)** – Execute a SQL statement and return a `Result`.

- **sqlalchemy.orm.Session.scalars(statement, params=None, \*\*kw)** – Execute a statement and return scalar results.

- **sqlalchemy.orm.sessionmaker(\*\*options)** – Factory for creating configured `Session` objects.
...
- **sqlalchemy.orm.mapped_column(\*args, primary_key=False, nullable=None, default=None, \*\*kw)** – Declare an ORM‑mapped column with typing support.

- **sqlalchemy.orm.relationship(argument=None, \*\*options)** – Define ORM relationships between mapped classes.

- **sqlalchemy.orm.joinedload(attr, \*\*kw)** – Eager load a relationship using a JOIN.

- **sqlalchemy.ext.associationproxy.association_proxy(target_collection, attr, \*\*kw)** – Create an association proxy for simplified many‑to‑many access.

- **sqlalchemy.orm.Query** ⚠️ – Legacy query API (soft‑deprecated; prefer `select()` for 2.0 style).  
...
- **Bound parameters everywhere**: Never render literal values in SQL strings; use bound parameters to prevent SQL injection and enable query‑plan caching.
...
- **Eager loading control**: Choose loading strategies (`joinedload`, `selectinload`, `subqueryload`, `lazyload`) per query.
...
- **Removed `future=True`**: The `future` flag is no longer needed and is deprecated in SQLAlchemy 2.0. Calls to `create_engine(..., future=True)` should be updated to omit the argument.
- **Deprecated legacy Query API**: `session.query()` and the `Query` class are soft‑deprecated. Migrate to `Session.execute(select(...))` and use `.scalars()` or `.unique()` as appropriate.
- **Sharding API updates**: `id_chooser` and `query_chooser` are deprecated; use `identity_chooser` and `execute_chooser` instead.
- **Typed ORM mappings**: Prefer `DeclarativeBase`, `Mapped[…]`, and `mapped_column()` over the older `declarative_base()` and plain `Column` usage.
- **Explicit transactions**: Implicit autocommit has been removed; always wrap Core writes in `engine.begin()` and ORM writes in a `Session` with an explicit `commit()`.