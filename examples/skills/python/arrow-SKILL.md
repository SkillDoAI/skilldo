...
- **arrow.parser.ParserError** - Exception raised for parsing failures.
...
- **DateTimeParser.parse(datetime_string: str, fmt: Union[str, List[str]], normalize_whitespace: bool = False) -> datetime** - Parse a datetime string with given format(s).
- **DateTimeParser.parse_iso(datetime_string: str, normalize_whitespace: bool = False) -> datetime** - Parse ISO format datetime strings.
