...
# Explicit imports for classes that are not re‑exported at the top level in this environment
...
if __name__ == "__main__":
    asyncio.run(main())
```
* Use `async with ClientSession()` to ensure the connector and sockets are closed.
...
async def main() -> None:
    # context object is created per-request by this factory
    ...
    async with ClientSession(trace_configs=[trace_config]) as session:
        # Per-request context fields can be initialized via trace_request_ctx mapping
...
from aiohttp.client import ClientSession  # separate session without middlewares
...
class TokenRefreshMiddleware:
    ...
    async def _refresh_access_token(self, session: ClientSession) -> None:
        async with self._refresh_lock:
            # Use a separate session without middlewares to avoid recursion
            async with ClientSession() as refresh_session:
                async with refresh_session.post(
                    self.token_endpoint,
...
async def main() -> None:
    token_middleware = TokenRefreshMiddleware(
        "http://localhost:8080/token", "<refresh-token>"
    )
...
* Use a separate `ClientSession` (without middlewares) inside the middleware when making internal calls to avoid recursion.
...
async def background_task(app: web.Application) -> None:
    while not app.get("_shutdown", False):
...
* **Client timeouts**: configure via `ClientTimeout(total=..., connect=..., sock_read=..., sock_connect=..., ceil_threshold=5)` and pass to `ClientSession(timeout=...)`.
...
def create_app() -> web.Application:
    ...
    app.router.add_get(r"/{name:[A-Яа-я]+}", handler)
    return app
```
...
**Fix:** Since aiohttp request methods (`get`, `post`, etc.) do **not** accept a `middlewares` argument, the pattern above is invalid. The correct approach is to perform internal calls using a separate `ClientSession` that was created without middlewares, as shown in the `TokenRefreshMiddleware` example above.
...
- **aiohttp.ClientTimeout(total=None, connect=None, sock_read=None, sock_connect=None, ceil_threshold=5)** - Timeout configuration for requests.
...
- **aiohttp.WSTimeout(...)** - WebSocket timeout configuration helper (client‑side).
...
- **aiohttp.web.Response(text=None, body=None, status=200, reason=None, headers=None, content_type=None, charset='utf-8')** - Basic HTTP response.
...
- **aiohttp.web.AppKey(name, t=None)** - Type‑safe key for storing application state.
...
- **aiohttp.BaseConnector(...)** - Base connector type (advanced usage; typically use `TCPConnector`).

### Authentication
- **aiohttp.BasicAuth(login, password="", encoding="latin1")** - HTTP Basic authentication helper; pass to `auth=` parameter.
- **aiohttp.DigestAuth(username, password)** - HTTP Digest authentication helper for client requests.
...
- **aiohttp.parse_content_disposition(...)** - Parse Content‑Disposition header into `(disposition_type, params)`.
- **aiohttp.content_disposition_filename(...)** - Extract filename from Content‑Disposition params (if present).
...
- **aiohttp.PAYLOAD_REGISTRY** - Global payload registry (advanced/extensibility).

### Testing Utilities
- **aiohttp.test_utils.TestServer(app, host="127.0.0.1", port=0, scheme="")** - Test server wrapper; `await start_server()` and `make_url(path)`; supports `async with`.
...
- **aiohttp.test_utils.make_mocked_request** - Create mocked `web.Request` for unit testing without server.
- **aiohttp.test_utils.unused_port()** - Get an unused port number for tests (best‑effort).
- **aiohttp.test_utils.make_mocked_coro(...)** - Build an awaitable Mock for unit tests.
