...
ecosystem: python
# license: Unknown
generated_with: qwen3-coder:latest
---
...
# Add/subtract time using shift (new API)
tomorrow = arr.shift(days=1)
last_week = arr.shift(weeks=-1)
```
...
from datetime import datetime
dt = datetime(2024, 1, 15, 14, 30, 0)
arr = arrow.get(dt, 'UTC')  # Explicit timezone or inferred from dt if aware
```