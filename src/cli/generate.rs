...
pub async fn run(opts: GenerateOptions) -> Result<()> {
    ...
    if let Some(ref provider) = provider_override {
        ...
        // default if the current value is "none" or from a different provider.
        // This prevents a local config's api_key_env = "none" from blocking a
        // CLI-specified provider that needs a key.
