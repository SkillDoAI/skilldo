...
impl ContainerExecutor {
    ...
    fn generate_java_install_script(&self, deps: &[String]) -> Result<String> {
        ...
        Ok(format!(
            "mkdir -p deps m2-repo\ncat > pom.xml << 'POMEOF'\n{pom}\nPOMEOF\nmvn dependency:copy-dependencies -DoutputDirectory=deps -Dmaven.repo.local=m2-repo -q 2>maven-errors.log || {{ echo 'WARNING: Maven dependency resolution failed — tests may fail due to missing jars'; cat maven-errors.log; }} >&2"
        ))
    }
