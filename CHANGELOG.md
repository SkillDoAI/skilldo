...
- Added `provider_type` as the preferred config field name (`provider` still accepted as legacy alias, to be removed in 0.5.0)
- Added `provider_name` field for human-readable provider labels and OAuth token storage keys
- Added `GoExecutor` for bare-metal Go test execution (no longer requires container runtime for Go)
- Added Go e2e smoke test in CI (chi v5.2.1, generates + lints + validates Go SKILL.md)
- Added typed timeout errors via `thiserror` — replaces fragile `e.to_string().contains("timed out")` string matching
- Added `SkillDoError::Timeout` variant with `downcast_ref` instead of string matching in retry logic
...
- Updated README badges, pitch deck, Go language support status, model references
